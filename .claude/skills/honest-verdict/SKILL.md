...
7. **组合视角（边际价值，判决补充）**：新策略判活前补问——① 它与现有**活策略**
   （当前仅 B2_4h）相关吗？② 它吃的是不是**同一个 regime**？依据：MR 死与趋势活是
   **同一 regime 事实的两面**（顺势收割的正是逆势被吃的那条右偏尾——出处
   `research/_closed/_synthesis/research_catalog_trend_mr_20260622.md` §4），在
   "延续 vs 反转"维度上并不正交；故"又一个吃延续的加密永续策略"对组合的**边际
   价值有限**，即使它单独看有 edge。
   **边界（重要）**：这是判决时的**一个视角，不是建组合层**。当前仅一个活策略，本判据
   现阶段是**记账**（写进判决书，不改单策略生死）；待有**第二个可配置策略**时，再建
   **独立的组合评估**（届时才需相关性 / 风险贡献 / 回撤重叠等完整工具）。
