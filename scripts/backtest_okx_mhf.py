...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--start",
        default="2025-01-01",
        help="Backtest start datetime in ISO format. Date-only means 00:00:00 local DB timezone.",
    )
    parser.add_argument(
        "--end",
        default="2026-03-31",
        help="Backtest end datetime in ISO format. Date-only means 23:59:59.999999 local DB timezone.",
    )
    parser.add_argument(
        "--capital",
        type=float,
...
def parse_local_datetime(value: str, is_end: bool) -> datetime:
    """Parse local naive datetime for vn.py sqlite backtesting."""
    ...
    if len(text) == 10:
        try:
            ...
            raise ConfigurationError(f"无法解析日期参数: {text}") from exc
        return datetime.combine(parsed_date, dt_time.max if is_end else dt_time.min)

    try:
        parsed_dt = datetime.fromisoformat(text)
    except ValueError as exc:
        raise ConfigurationError(f"无法解析时间参数: {text}") from exc
...
def main() -> int:
    ...
    try:
        ...
        start_dt = parse_local_datetime(args.start, is_end=False)
        end_dt = parse_local_datetime(args.end, is_end=True)

        strategy_config = load_json_file(STRATEGY_CONFIG_PATH)
        instrument_meta = load_instrument_meta(INSTRUMENT_CONFIG_PATH)
        validate_runtime_config(args.vt_symbol, instrument_meta, start_dt, end_dt)

        raw_setting = strategy_config.get("setting")
        if not isinstance(raw_setting, dict):
            raise ConfigurationError(f"strategy_default.json 缺少 setting 对象: {STRATEGY_CONFIG_PATH}")

        strategy_setting = fill_strategy_setting(raw_setting, instrument_meta, args.capital, logger)
        strategy_class_name = str(strategy_config.get("class_name", "")).strip()
        if not strategy_class_name:
            raise ConfigurationError(f"strategy_default.json 缺少 class_name: {STRATEGY_CONFIG_PATH}")
        ...
        daily_output_df = daily_df.reset_index()
        ...
        stats_payload.update(
            {
                "warning": WARNING_TEXT,
                "trade_count": statistics.get("total_trade_count", 0),
                "win_rate": round_trip_stats.win_rate,
                "profit_loss_ratio": round_trip_stats.profit_loss_ratio,
                "closed_trade_count": round_trip_stats.closed_trade_count,
        ...
        export_dataframe_csv(daily_output_df, output_dir / "daily_pnl.csv")
        export_records_csv(
            trade_records,
        ...
        if figure is not None:
            figure.write_html(str(chart_path), include_plotlyjs=True, full_html=True)
        else:
        ...
        write_json_file(output_dir / "stats.json", stats_payload)
        ...
        summary = {
            ...
            "trade_count": stats_payload.get("trade_count"),
            "closed_trade_count": stats_payload.get("closed_trade_count"),
        }
