#!/usr/bin/env python3
"""Exit-Only Research: compare exit mechanisms with fixed simple entry.

This script is research-only. It uses a fixed 20-bar breakout entry on 4h bars,
then compares different exit mechanisms (baseline ATR stop, trend drawdown
tolerance, 1d trend health lock, and B+C combined) across train_ext /
validation_ext / oos_ext splits with no-cost, cost-aware, and funding-adjusted
PnL layers.

Exit variants:
  A: Baseline - 2x ATR stop loss, 60-bar max hold
  B: Trend drawdown tolerance - exit when drawdown from peak > 2x entry_ATR
  C: 1d trend health lock - suppress exit when 1d 20-EMA confirms direction
  B+C: Combined B + C
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "reports" / "research" / "exit_only_bc"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Exit-Only Research: B + C exit mechanisms.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger = setup_logging("research_exit_only_bc", verbose=bool(args.verbose))
    ...
    # Summary JSON
    summary_json = {
        "hypothesis": "Exit-Only Research: trend drawdown tolerance (B) + 1d trend health lock (C)",
        "version": "v1.0",
    ...
    write_json(output_dir / "exit_only_bc_summary.json", summary_json)

    # Markdown report
    report = _render_report(summary_json, by_variant, concentration, reverse_summary)
    (output_dir / "exit_only_bc_report.md").write_text(report, encoding="utf-8")

    log_event(logger, logging.INFO, "research_complete", "Exit-Only B+C research complete",
