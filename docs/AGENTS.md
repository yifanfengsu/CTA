...
- gate 判定由预注册文档自动裁决（K1/K2/K3 KILL、U1/U2 UPGRADE），
  不做临场解释。
- **记账显示口径一致性**：任何面向人的报告（日报 / 仪表盘）展示的核心指标
  （累计盈亏 / 回撤 / gate 余量），必须与 gate 判定所用的**同一前向切片、同一净值
  序列、同口径**。禁止显示层用独立的减法 / 提取 / 重算逻辑另算；也禁止把指标标成
  名不副实的名字（如把 MAX 回撤标成"当前回撤"、把 `ceil` 末桶标成"当日"）——显示层
  与判定层口径分叉会产出**自相矛盾且难察觉**的数字，而核心账与 gate 却可能完好。
  诊断时先用 audit-independent 独立重推区分"显示层混淆"与"gate 层污染"，再决定是否
  触碰系统（见 prior-registry `forward_report_display_caliber` /
  `reports/forward_accounting_audit_20260730/`）。
