...
- **第二研究：验证阶段（贝塔分离 + 5 项稳健性 gate）**（2026-06-11，
  `research/_closed/crypto_perp/trend_b2_4h/reports/trend_validation_20260611/`）：**VALIDATED 0 / FAIL 15**——V1 单笔集中度
...
- **第三研究：验证第二轮（V1' 集中度质量重审 + long/flat 族 D）**（2026-06-11，
  `research/_closed/crypto_perp/trend_b2_4h/reports/trend_validation_r2_20260611/`）：**VALIDATED\* 5 / FAIL 3**。
...
- **第四研究：组合构建（相关结构 + 4 个预注册组合过堂）**（2026-06-11，
  `research/_closed/crypto_perp/trend_b2_4h/reports/trend_portfolio_20260611/`）：**PORTFOLIO-CANDIDATE\* 0 / 4——全部死于
...
- **第五研究：双周期扩展验证（Binance 2020-2026 独立样本全量复测）**（2026-06-12，
  `research/_closed/crypto_perp/trend_b2_4h/reports/trend_dualcycle_20260611/`）：**DUAL-VALIDATED\* 2（B2_4h、D2）/
...
- **趋势线终局：资源决策关闭**（2026-06-12，`research/_closed/_synthesis/trend_line_closure_20260612.md`）：
  ...
  **期权 VRP（学习中）**，选题标准新增"验证周期"维度（已入 CLAUDE.md 工作原则）。
  ...
  <!-- 2026-06-22 校正（方法论加固，research/_closed/_synthesis/trend_methodology_hardening_20260622/）：
...
- **第六研究：价格回归的时间尺度结构（纯描述性前置研究，立项判定）**
  （2026-06-12，`research/_closed/crypto_perp/mr_timescale/reports/mr_timescale_structure_20260612/`）：**不立项，全尺度**。
...
- **第七研究：funding 结构前置研究（纯描述性，立项判定）**（2026-06-12，
  `research/_closed/crypto_perp/funding_reversal/reports/funding_structure_20260612/`）：**不立项**。极端费率事件（F1 首破
...
- **第八研究：波动率事件前置研究（纯描述性，立项判定）**（2026-06-13，
  `research/_closed/crypto_perp/volatility_event/reports/volatility_event_20260613/`）：**不立项，死因 b（精确形态：增量线+永续
...
<!-- 2026-07-12 更新：原平铺结构（scripts/ 126 脚本混放）被重构步骤2 取代，
     新结构见下方"2026-07 新结构"一节；旧树仅存于 git 历史与盘点报告
     reports/repo_audit_20260712.md -->

```
cta_strategy/
├── CLAUDE.md            # 项目最高约束（铁律/数据环境/工作原则）
...
│                        #   crypto_options/vrp_atm / basis / _synthesis（跨线综合）
├── audit/               # 审计域：CLAUDE.md 宪法 + scripts/（audit/postmortem/取证 12 个）
├── scripts/             # ⚠️ 前向冻结区：forward_b2_4h.py + import 闭包共 11 个 .py
│                        #   （两引擎+tv/dc/基准引擎/research_mr_5m/common_runtime/
│                        #    history_time_utils/binance_funding/okx_vs_binance）
├── forward/             # ⚠️ B2_4h 前向观察系统（VPS 运行中，整目录冻结）
├── strategies/          # vnpy CTA 策略模板（冻结区之外的历史策略本体，未动）
├── config/              # 合约规格、运行时配置、策略参数（未动）
├── data/                # 数据资产（未动；前向 seed 依赖 data/funding/okx/）
├── reports/             # 存档产物：regime 取证链、大宗历史目录、盘点报告
├── _archive/            # 遗留归档：mr5m_runner/（9+restart.sh）、legacy_scripts/
│                        #   （55 个 demo 时代脚本）、legacy_reports/（原 reports/research/）
├── tests/               # 单元测试（tests/__init__.py 集中注入各脚本目录路径）
├── logs/ deploy/        # 运行日志 / 部署占位（未动）
├── .vntrader/           # vnpy 引擎数据（mainnet 库 + 污染库，只读，未动）
└── Makefile             # 统一入口（路径已随重构更新；历史 target 见头部横幅说明）
```

## 2026-07 新结构（重构步骤2，2026-07-12）

**各域职责一览**（宪法与路由详见 `docs/AGENTS.md`）：

| 域/区 | 职责 | 位置 |
|---|---|---|
| data_engineering | 下载/校验/入库/manifest，数据真实性可证伪核对 | `data_engineering/scripts/` |
| research | 前置研究/回测/生死判定（走 PIPELINE 九站）；已关闭线收拢于 `_closed/` | `research/` |
| audit | 核算诚实性/结论复核/数据取证（独立重推，只核对不修改） | `audit/scripts/` |
| forward | B2_4h 零成本前向观察（VPS cron 自主运行，Claude 只动文档） | `forward/` + `scripts/` 冻结区 |
| core | 市场无关共享库（数据 IO / DB 工具） | `core/` |
| _archive | demo 时代与已关停资产（历史证据，只归档不删除） | `_archive/` |
...
| `README_BOOTSTRAP.md` | `docs/BOOTSTRAP.md` |
| 旧 `README.md`（1521 行流水账） | `docs/README_ARCHIVE.md`（原文存档） |
| `scripts/history_utils.py` | `core/data_io/history_utils.py` |
| `scripts/init_db.py`、`scripts/doctor.py` | `core/db/` |
| `scripts/common_runtime.py`、`scripts/history_time_utils.py` | **原位冻结**（前向闭包）；`core/data_io/` 有同名 re-export 代理 |
| `scripts/download_*` `verify_*` `import_*` `probe_*` `refresh_*` `merge_recent` `research_funding_trust_check` `check_okx_connection` `inspect_okx_gateway` | `data_engineering/scripts/` |
| `scripts/audit_*`、`scripts/postmortem_*`、`forensics_data_contamination.py` | `audit/scripts/` |
| `scripts/research_<已关线>*.py`（trend 增强/mr_timescale/pairs/funding/cross_sectional/breakout/volatility/tick 三线/basis） | `research/_closed/<market>/<line>/scripts/` |
| `reports/<line>_<date>/`（12 条线的研究报告目录） | `research/_closed/<market>/<line>/reports/` |
| `vrp/` | `research/_closed/crypto_options/vrp_atm/` |
| `reports/MR5M_postmortem.md` 等 7 项顶层收尾文档 | `research/_closed/_synthesis/` |
| `reports/research/`（demo 时代黑洞） | `_archive/legacy_reports/` |
| `scripts/run_mr_5m_direct.py` 等 9 个 runner + `restart_mr5m.sh` | `_archive/mr5m_runner/`（restart 在 `_archive/` 根） |
| 55 个 demo 时代研究/回测/分析脚本 | `_archive/legacy_scripts/` |
| **不变**：`scripts/` 冻结 11 件、`forward/`、`.vntrader/`、`data/`、`config/`、`strategies/`、`.env`、`reports/regime/`（事故证据链）、`reports/` 大宗历史目录（ablation/alpha_sweep/backtest*/等） | — |

**迁移不变量**（重构步骤2 的验收基线）：前向三件套及其 import 闭包共 11 文件
零改动；`forward/config_frozen.json` SHA256 `001b0d9e…0012e6` 不变；tests 红集
与重构前基线逐项一致（401 tests，7F+1E 均为既有红：硬禁用下载器防呆 +
audit_extended_history 历史漂移）；全部迁移 git mv 保留历史。
...
- **15/15 未通过含原 V1 的首轮验证；V1' 重审后 5 配置 VALIDATED\*（2026-06-11）**：
  ...
  [`research/_closed/crypto_perp/trend_b2_4h/reports/trend_validation_20260611/` + `trend_validation_r2_20260611/`]
- **趋势幸存池 ≈2 个独立信号；组合层无统计支持（2026-06-11）**：5 个 VALIDATED*
  ...
  （前者持仓为后者多头腿子集）。[`research/_closed/crypto_perp/trend_b2_4h/reports/trend_portfolio_20260611/`]
- **B2_4h 与 D2 通过 Binance 2020-2026 双周期独立复测（2026-06-12）**：
  ...
  后净额，冻结数字未因漏扣 funding 虚高；`research/_closed/crypto_perp/trend_b2_4h/reports/b2_4h_pnl_audit_20260628/`）、
  whipsaw 让 long/flat 多亏 ~$19k →
  含 short 腿的 B2_4h 为首选形态。两所重叠期偏差仅 +1~7%。注意：此结论
  回答"信号在加密市场稳健"，不推翻组合阶段对 2023+ 日收益显著性的否定。
  [`research/_closed/crypto_perp/trend_b2_4h/reports/trend_dualcycle_20260611/`]
- **趋势线终局记录（2026-06-12）**：趋势线以**资源决策**关闭，非信号证伪——
  ...
  重启条件已预留。[`research/_closed/_synthesis/trend_line_closure_20260612.md`]
  <!-- 2026-06-22 校正（research/_closed/_synthesis/trend_methodology_hardening_20260622/）：本条"~15-19 年"
...
- **B2_4h funding 同向确认无效，趋势线维持资源关闭（双样本，2026-06-14）**：
  ...
  预测力（趋势确认与 MR 极值反转两路均否）。[`research/_closed/crypto_perp/trend_b2_4h/reports/trend_funding_confirm_20260613/`]
- **B2_4h ADX(14)>25 趋势强度过滤无效且有害、机制反向（双样本，2026-06-16）**：与 funding
  ...
  在"OKX 单边描线"与"双样本一致反向"两种失败模式下都正确判死。[`research/_closed/crypto_perp/trend_b2_4h/reports/trend_adx_filter_20260616/`]
- **更早入场 EMA10/30 双样本每轴劣于 B2_4h，机制=快的对称性（2026-06-16）**：由 ADX"70% 利润在
  ...
  全败，"更早入场"悬念终结，趋势线维持资源关闭**；防线栈三种失败模式下均正确判死。[`research/_closed/crypto_perp/trend_b2_4h/reports/trend_faster_entry_20260616/`]
- **15m–4h 价格回归无可收割期望（双样本，2026-06-12）**：回归作为频率现象
  ...
  尺度行"的中间地带；中频 MR 线不立项。[`research/_closed/crypto_perp/mr_timescale/reports/mr_timescale_structure_20260612/`]
- **极端 funding 费率不构成正期望回归结构（双样本，2026-06-12）**：正极端
  ...
  确认因子（未检验，属新开题）。[`research/_closed/crypto_perp/funding_reversal/reports/funding_structure_20260612/`]
- **15m 突破后"50% 回调接延续"是民俗，无结构（双样本，2026-06-12）**：全部
  ...
  [`research/_closed/crypto_perp/breakout_pullback/reports/breakout_pullback_15m_20260613/`]
- **波动率可预测但永续不可货币化，需期权载体（双样本，2026-06-13）**：波动率聚集
  ...
  唯一重启线索，记录不行动）。[`research/_closed/crypto_perp/volatility_event/reports/volatility_event_20260613/`]
- **加密横截面信号（动量/carry/反转）在幸存者样本上不立项（2026-06-13）**：22 币纯加密
  ...
  **真 alpha 是必要非充分——横截面成功对冲方向但本市场该 alpha 太弱+太不稳。** 幸存者
  ...
  [`research/_closed/crypto_perp/cross_sectional/reports/cross_sectional_ic_20260613/`]
  <!-- 2026-06-13 carry 持有期重看（research/_closed/crypto_perp/cross_sectional/reports/cross_sectional_carry_holding_20260613/）：
...
<!-- 2026-07-12 更新：scripts/ 平铺结构被重构步骤2 取代——scripts/ 现为前向
     冻结区（11 个 .py），本节所列脚本的现位置见上方"2026-07 新结构"速查表
     （runner→_archive/mr5m_runner/，demo 研究→_archive/legacy_scripts/，
     数据工程→data_engineering/scripts/，审计→audit/scripts/，
     已关线研究→research/_closed/）。本节分类描述保留为历史记录。 -->
...
> 详见 `research/_closed/_synthesis/MR5M_postmortem.md` 第 7 节。
...
- 当前研究状态：**永续信号空间已闭环**——"永续(线性)×≤4h×零售 taker×5 币价格+funding"约束盒内，方向式/回归式/波动率式三大类单序列信号机制已全部双样本证否或证明验证不起。**完整地图、五条出路、重启索引见 `research/_closed/_synthesis/perpetual_signal_space_closure_20260613.md`（阶段性收口文档，第一参照）。** 其下：MR-5m 项目已关闭；趋势线已关闭（2026-06-12 资源决策，非证伪，重启条件见 `research/_closed/_synthesis/trend_line_closure_20260612.md`）；已检验未立项候选 = MR 时间尺度结构 / funding 结构 / 15m 突破回调（死因 c）/ 波动率事件（死因 b/增量线+永续线性壁，需期权载体，`research/_closed/crypto_perp/volatility_event/reports/volatility_event_20260613/`）。离盒出路中横截面/相对价值曾是唯一"约束集内、无需新载体/身份"的通顺主线，但其 IC 前置研究（2026-06-13，`research/_closed/crypto_perp/cross_sectional/reports/cross_sectional_ic_20260613/`）三因子全灭、**在幸存者样本上即不立项**——横截面方向就此关闭。剩余离盒出路均需改约束盒（期现基差/对冲 funding 需现货数据；做市需 maker 身份+微观结构；期权需新载体），见 `research/_closed/_synthesis/perpetual_signal_space_closure_20260613.md` §5。**其中期现基差已执行前置研究并不立项（2026-06-16）：数据可得但扣费后净基差年化≈无风险、超额≈0，且空头交割腿最坏逆向+80%需 cross-margin，加密版固收薄到不值得双腿工程，见 `research/_closed/basis/basis_arbitrage/reports/basis_arbitrage_feasibility_20260615/` 与上「已验证的核心事实」。**<!-- 2026-06-16 更新：原"期现基差需现货数据"为未测出路，现已测：数据绿灯但无超额，不立项 -->**均值回归家族（单序列价格 + 横截面 carry + 配对价差 + funding 极端）三支/四线全部探明完毕、整体收口（2026-06-13，`research/_closed/crypto_perp/pairs_cointegration/reports/pairs_cointegration_20260613/`）——回归式信号在永续数据上无一立项。**<!-- 2026-06-11 更新：原"唯一在跑方向：动态仓位（双账号对照实验）"随项目关闭而终止 --><!-- 2026-06-12 更新：原"新阶段为新策略研究"细化——第一条线（趋势）已走完五阶段，以资源决策关闭 -->
