...
- [x] **数据源验证**：任何写库脚本启动时必须向 stdout 打印数据环境（MAINNET/DEMO）；manifest 必须含 `server` 字段；新数据入库后、用于研究前，与至少一个独立外部源抽样交叉验证（如 Binance vision 月度文件，抽 ≥3 个随机日逐 bar 对比方向与量级——注意 Binance 是不同交易所价格，验的是"同一个市场"，不是逐 tick 相等）。<!-- 2026-06-11：对当前 database_mainnet.db 已闭环——Binance 全量（非抽样）交叉验证 PASS + funding 正式核查通过，见 reports/regime/data_trust_closure_20260611/。未来任何新数据仍须逐批重做本项。 -->
